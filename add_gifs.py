...
def save_collection(data: Dict[str, Any], path: str = COLLECTION_PATH) -> None:
    """Save collection metadata JSON atomically, then sync into SQLite."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
    ...
    # Keep the SQLite database (used by app.py) in sync automatically
    try:
        from data import db as gifdb
        gifdb.sync_from_json(json_path=path)
    except Exception as e:
        print(f"⚠️ Warning: could not sync SQLite database: {e}")
